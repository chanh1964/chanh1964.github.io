...
const generateDetail = (data: Update) => {
  if (!data.detail) return null;
  else {
    const span_detail = [];
    for (const line of data.detail) {
      span_detail.push(
    ...
    if (data.links) {
      ...
      for (const link of data.links) {
        links.push(
          <a className="chanh-link break-all block" href={link} target="_blank">
            {link}
          </a>
        );
      }
...
const generateDetailWithThumbnail = (data: Update) => {
  return (
    <Row gutter={16} className="mt-2 !mx-0">
      ...
      <Col lg={16}>{generateDetail(data)}</Col>
    </Row>
  );
};

export const generatePostContent = (data?: Update) => {
  if (!data) return;
  return (
    <>
      <i>
      ...
      {/* </span> */}
      {data.thumbnail_link
        ? generateDetailWithThumbnail(data)
        : generateDetail(data)}
    </>
  );
};
...
const ChanhDetailModal = (props: Props) => {
  const showDetail = () => {
    return Modal.info({
      ...
      closable: true,
      footer: [],
      content: generatePostContent(props.data),
